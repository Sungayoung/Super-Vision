...
import VideoResultCard from '../../../Components/Cards/VideoResultCard';
...
function ImageFilterExperience () {
  ...
  const [normalVideo, setNormalVideo] = useState<string>('')
  const [srVideo, setSrVideo] = useState<string>('')
  ...
  const title: string = "TRY SUPER RESOLUTION on VIDEO"
  const content: string = "비디오를 업로드해서 일반 필터와 AI 필터의 화질 개선을 직접 확인하세요!\n화질이 개선된 비디오는 다운로드가 가능합니다" 


  function parentVideoChange (normalVideo: string, srVideo: string ): void {
    setNormalVideo(normalVideo)
    setSrVideo(srVideo)
  }
          ...
          <Tooltip title="돋보기 켜기">
            <span>
              <CustomSwitch
                ...
                disabled={!Boolean(normalVideo) || !Boolean(srVideo)}
              />
            </span>
          </Tooltip>
            <div className="cards">
              <VideoUploadCard parentVideoChange={parentVideoChange} />
              <ArrowRightIcon className="mt-5 mx-4" sx={{ color: '#F2FFFF', fontSize: 50}}/>
              <VideoResultCard title="일반 필터" file={file} videoPreviewUrl={normalVideo}  setMousePos={sendMousePos} pos={mousePos}/>
              <VideoResultCard title="AI 필터" file={file} videoPreviewUrl={srVideo}  setMousePos={sendMousePos} pos={mousePos}/>
            </div>
        </div>
      </div>
    </>
