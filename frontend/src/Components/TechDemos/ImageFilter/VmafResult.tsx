...
function VmafResult ({normalVmaf, srVmaf, diff}: VmafResultProps) {
  ...
  return (
    <>
      {srVmaf > 0 ? <div className="card-container d-flex flex-column text-center justify-content-between">
          <div className="mb-2 font_2 main_color bold">VMAF SCORE</div>
          <div className="">
              <BarChart data={[normalVmaf, srVmaf]} labels={['일반 필터', 'AI 필터']} colors={colors} />
          </div>
      </div> : ''}
    </>
  )
